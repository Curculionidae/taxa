...
**Citations** are fetched by the lightbox itself (`imageCitations.js`, lazily for the shown image ± 1) unless the caller already put a non-empty `citations` array on the image object. Rendered as `Citation: <short ref>` (`Citations:` when >1); the short ref is a `<button>` that opens the `Reference` modal (`source.cached`, else the short ref). That modal is a **direct child**, not `<Teleport>`ed — VModal's overlay is only `z-[2000]` and would sit behind this `z-[10000]` viewer from `<body>`; as a child it stacks inside the viewer's context (same as the ⓘ `DwcTable`). While it is open the viewer ignores keys so Escape/←/→ act on the modal.
